...
RUN cd boost_${BOOST_VERSION} \
    && ./b2 --build-type=complete link=static runtime-link=static --with-chrono --with-date_time --with-filesystem --with-program_options --with-regex --with-serialization --with-system --with-thread --with-locale --build-dir=android64 --stagedir=android64 toolset=clang threading=multi threadapi=pthread target-os=android stage
...
RUN git clone https://github.com/samvicky26/worktipstest.git \
    && cd worktipstest \
    && mkdir -p build/release \
    && cd build/release \
    && CC=clang CXX=clang++ \
         BOOST_ROOT=${WORKDIR}/boost_${BOOST_VERSION}/ \
		 BOOST_LIBRARYDIR=${WORKDIR}/boost_${BOOST_VERSION}/android64/lib/ \
